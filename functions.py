...
def process_ticker(ticker, start_date, end_date):
    ...
    daily_returns = np.array(adj_close.pct_change().dropna() * np.sqrt(365.25))
    ...
    for name, distribution in distributions.items():
        ...
        hist, bin_edges = np.histogram(filtered_data, bins=50, density=True)
        bin_centers = 0.5 * (bin_edges[1:] + bin_edges[:-1])
        fitted_pdf = distribution.pdf(bin_centers, *params)

        mape = np.mean(np.abs(hist - fitted_pdf)/np.abs(fitted_pdf)) * 100 # Mean Absolute Error
